...
- 日内趋势图中：
  - 不显示均线相关信息（均线属于历史趋势图）；但为了避免切换时抖动，日内 tab 在顶部占位（空白）与历史 tab 保持一致空间。
  - 日内图下方的点位信息区域（时间 / 净值 / 与上一日涨跌）保持固定列宽（如 w-36、w-44、w-48），避免 hover 时跳动。
  - 不在日内图中显示横纵坐标刻度数值（只显示轻微的 y 网格线和 x 轴的 0/中/末 时间标签以便对齐）。悬停交互以历史趋势图类似方式展示 hover 信息（竖直虚线 + 顶端日期显示）。
  - 图内最后一个点使用高亮样式（比其它点更大）以便识别当前最新值。
