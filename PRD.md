...
## 风险监控中心 (Risk Monitor Center) — 新增功能说明 (v1.42)

风险监控中心是实时监控用户投资组合风险状况的功能模块，提供风险评分、预警列表、集中度分析、回撤追踪等多维度风险监控能力。

### 界面入口
- 在主界面顶部菜单栏中，"持仓"按钮右侧新增"风险"按钮（图标使用警示图标如 `fa-exclamation-triangle`），点击后打开风险监控中心窗口。

### 界面布局
- 窗口形式：居中模态框，尺寸与 `OverallProfitModal` 一致（max-w-4xl，约64rem宽，90vh高）
- 关闭方式：点击关闭按钮或点击背景
- 布局结构：
  - 左侧：功能导航栏（200px宽度），包含4个Tab导航项
  - 右侧：内容展示区，根据选中的Tab显示对应内容

### Tab导航项
1. **风险总览** - 综合风险评分 + KPI指标卡片 + 预警预览
2. **预警列表** - 预警详情列表（按严重程度排序）
3. **集中度分析** - 持仓分布饼图 + 集中度指标 + 分散化建议
4. **回撤追踪** - 当前回撤状态 + 恢复进度 + 各基金回撤表

### Tab 1: 风险总览
#### 综合风险评分
- 顶部居中显示大号数字（如"72分"），配色根据分数区间变化：
  - 0-39分：红色（高风险）
  - 40-59分：橙色（中等风险）
  - 60-79分：黄色（中低风险）
  - 80-100分：绿色（低风险）
- 分数下方显示风险等级文字（高风险/中等风险/中低风险/低风险）
...
- 展示5个核心风险指标卡片（网格布局）：
  ...
  - **卡玛比率**：年化收益与最大回撤比值
  - **集中度（HHI）**：持仓集中度指数
- 每个卡片支持hover tooltip，显示指标解释和当前阈值

#### 预警预览
- 底部显示最近3条预警摘要，点击可跳转到"预警列表"Tab
...
#### 持仓分布饼图
- SVG饼图，展示各基金持仓占比
- Hover扇区显示：基金名称、市场价值、占比
- 点击扇区可跳转到对应基金详情页

#### 集中度指标卡片
- **单基金最高占比**：当前值 + 阈值对比
- **前三基金合计占比**：当前值 + 阈值对比
- **HHI指数**：集中度综合指标（0-1，越低越分散）
- **有效持仓数**：非零持仓基金数量

#### 分散化建议
- 当集中度超标时，显示具体建议（如"建议降低XX基金仓位至XX%"）
...
#### 各基金回撤表
- 表格列：基金名称、当前回撤、最大回撤、回撤起始日
- 按当前回撤降序排列

### 计算策略

#### 增量计算
- 采用增量计算策略，避免每次打开窗口时全量计算
- 缓存策略：仅内存缓存，不持久化到 localStorage
- 变化检测：事件驱动 + 数据指纹校验
...
阈值配置集成到「系统配置 → 系统参数」面板中，位于 OCR 并发设置下方。

#### 回撤预警阈值
| 参数 | 默认值 | 范围 | 说明 |
|------|--------|------|------|
| 轻度预警 | 10% | 5%-20% | 回撤超过此值触发轻度预警（黄色） |
| 中度预警 | 15% | 10%-30% | 回撤超过此值触发中度预警（橙色） |
| 重度预警 | 20% | 15%-40% | 回撤超过此值触发重度预警（红色） |

#### 波动率预警阈值
| 参数 | 默认值 | 范围 | 说明 |
|------|--------|------|------|
| 低波动阈值 | 15% | 5%-20% | 波动率低于此值为低风险（绿色） |
| 高波动阈值 | 25% | 20%-40% | 波动率高于此值为高风险（红色） |

#### 单日波动阈值
| 参数 | 默认值 | 范围 | 说明 |
|------|--------|------|------|
| 预警阈值 | 3% | 1%-10% | 单日涨跌超过此值触发预警 |
| 严重阈值 | 5% | 1%-15% | 单日涨跌超过此值触发严重预警 |

#### 连续下跌阈值
| 参数 | 默认值 | 范围 | 说明 |
|------|--------|------|------|
| 轻度关注 | 3天 | 1-10天 | 连续下跌达到此天数触发关注 |
| 高度关注 | 5天 | 1-15天 | 连续下跌达到此天数高度预警 |

#### 集中度预警阈值
| 参数 | 默认值 | 范围 | 说明 |
|------|--------|------|------|
| 单基金上限 | 25% | 10%-50% | 单基金占比超过此值触发预警 |
| 前三基金上限 | 70% | 30%-90% | 前三基金合计占比超限触发预警 |

### 综合风险评分算法

风险评分（0-100分）由以下因素加权计算：

```
风险评分 = 40% × 回撤得分 + 25% × 波动得分 + 20% × 集中得分 + 15% × 连跌得分
```

各分项得分计算：
- **回撤得分**：根据当前回撤占重度预警阈值的比例计算
- **波动得分**：根据当前波动率占高波动阈值的比例计算
- **集中得分**：根据HHI指数计算
- **连跌得分**：根据连续下跌天数占高度关注天数的比例计算

### 与绩效分析的关系

| 对比项 | 风险监控中心 | 绩效分析（整体盈利） |
|--------|------------|-------------------|
| 目的 | 实时风险预警 | 历史绩效回顾 |
| 数据范围 | 整体组合 + 单基金 | 整体组合 |
| 更新频率 | 打开时实时计算 | 打开时实时计算 |
| 指标维度 | 风险指标（回撤、波动） | 收益指标（年化收益、夏普） |

两者共享底层计算逻辑（`utils/performanceAttribution.ts`），但：
- 绩效分析：展示历史绩效曲线，支持日期范围选择
- 风险监控：实时监控当前风险状态，提供预警功能

### 实现文件

| 文件 | 说明 |
|------|------|
| `components/RiskMonitorModal.tsx` | 风险监控中心弹窗组件 |
| `services/riskCalculationService.ts` | 风险计算服务（增量计算） |
| `components/config/SystemParamsPanel.tsx` | 系统参数面板（新增风险阈值配置） |
| `types.ts` | 新增风险相关类型定义 |
| `utils/riskScoreCalculator.ts` | 综合风险评分算法 |
...
// 风险预警
interface RiskAlert {
  type: 'drawdown' | 'volatility' | 'concentration' | 'continuous_decline' | 'daily_change';
  level: 'low' | 'medium' | 'high';
  target: string;                   // 触发对象（基金代码或"整体组合"）
  currentValue: number;
  threshold: number;
...
interface RiskThresholds {
  drawdown: { low: number; medium: number; high: number };
  volatility: { low: number; high: number };
  dailyChange: { warning: number; severe: number };
  continuousDecline: { low: number; high: number };
  concentration: { singleFund: number; topThree: number };
}
...
| 测试文件 | 测试内容 |
|---------|---------|
| `tests/services/riskCalculationService.test.ts` | 增量计算、缓存策略、变化检测 |
| `tests/utils/riskScoreCalculator.test.ts` | 综合风险评分算法 |
| `tests/components/RiskMonitorModal.test.tsx` | UI 交互、Tab 切换、预警显示 |

投资计划AI辅助可以根据用户的基金持仓情况，以及基金本身的基本信息，自动制定今天的一揽子投资计划（基金）。

### 界面入口
* 在投资计划草稿窗口的工具栏上增加一个"AI辅助"图标（copilot相关的驾驶员图标），点击后将进行AI辅助决策后的投资计划自动填充到下方的表格中。
* 下方表格中，第一列多选按钮后面增加一列（"提示列"），不显示列名，在每行预留一个图标的位置。默认不显示。重置按钮在点击时，除了原有重置功能外，还应该重置提示列（清空）。

### 界面效果
* 在点击按钮之后，如果此时表格里面有用户输入的数据，那么在开始辅助决策之前，弹出警告对话框，警告用户所有数据将会被清空。用户可以选择继续或放弃（放弃操作，终止流程）。
* 用户确认后填充之前，先对表格原有填充数据清空，恢复初始状态。
* AI辅助决策完成后，弹出结果对话框，显示结束摘要（"辅助决策成功"或"辅助决策部分成功，达到最大尝试轮数"）。
* 将AI辅助决策返回的投资项目填充到对应的基金投资行上。每条有投资决策的行的"提示列"显示一个提示图标（小的蓝色感叹号）。图标上展示AI辅助决策的理由以及得分。得分低于阈值（0.7）时，得分数字显示为红色。
* 同时，注释框里面自动填充当时的涨跌幅。
...
* 数据来源：与"投资计划AI助手"的数据来源类似，除了下面的区别：
  * 不需要用户今天的投资计划数据，因为这是输出。
  * 基金的信息的范围是所有在表格上显示的所有基金。
* 输出内容：Json格式的结构化数据，包含需要交易的每个基金的代码，操作（买入/卖出），金额（元），解释。
* 理由限制在100字以内。

#### 迭代验证逻辑 (v1.41)
* 使用提示词模板1，向AI模型发送请求，获取投资计划的建议（invest_list）。
* 收到AI模型返回的结果后，将原始问题和AI模型的回答作为上下文，使用提示词模板2，向AI模型发送请求，要求得到对invest_list的每条投资计划的合理性得分（invest_with_score_list）。
* 如果所有投资计划的合理性得分都高于阈值（0.7），则计算结束，结束摘要为"辅助决策成功"。
* 如果有投资计划的合理性得分低于阈值，则在上下文中加入AI模型的合理性分析反馈，使用提示词模板3，要求AI对invest_list进行修改优化，得到新的invest_list，然后再次评分。如此循环，直到所有得分都高于阈值，或者达到最大迭代次数（3轮）为止。
* 达到最大迭代次数时，结束摘要为"辅助决策部分成功，达到最大尝试轮数"。

### 提示词模板
模板配置文件：`public/assets/config/ai-investment-draft-templates.json`（与投资计划AI助手共用同一配置文件）

| 模板ID | 用途 |
|--------|------|
| `ai-investment-advice` | 生成投资计划建议 |
| `ai-investment-advice-score` | 对投资计划进行合理性评分 |
| `ai-investment-advice-refine` | 根据评分反馈优化投资计划 |

模板支持的变量：
* `{json_schema}`：JSON数据的字段说明（不含用户计划字段）
* `{json_content}`：用户的持仓信息（JSON格式）

### 实现文件
| 文件 | 说明 |
|------|------|
| `services/aiInvestmentDraftService.ts` | 数据格式化和AI调用服务（复用投资计划AI助手的大部分代码） |
| `components/InvestmentDraftModal.tsx` | 投资计划草稿窗口，添加AI辅助按钮入口 |
| `components/SimpleTooltip.tsx` | 简单提示组件，用于显示AI决策理由和得分 |
| `public/assets/config/ai-investment-draft-templates.json` | 提示词模板配置 |

### 测试文件
| 文件 | 说明 |
|------|------|
| `tests/services/aiInvestmentDraftService.test.ts` | 服务层单元测试 |

---

## 投资计划AI助手 (AI Investment Draft Assistant) — 新增功能说明 (v1.39)
...
### AI分析浮窗
该浮窗的样式和功能与AI投资组合分析浮窗类似。

#### 数据来源
* 投资计划草稿窗口里面的操作为"买入"或者"卖出"的基金数据（今日投资计划）
* 基金的实时数据：当前净值、今日涨跌幅等
* 基金基本信息数据：近1周、近1月、近3个月、近6个月的涨跌幅，基金前十股票持仓等
* 基金历史数据：最近10天的净值、5日/10日/20日均值
* 指数实时数据：当前指数值、成交量等
* 指数历史数据：最近10天的指数值、成交量、均值等
...
### 提示词模板
模板配置文件：`public/assets/config/ai-investment-draft-templates.json`

模板支持的变量：
* `{json_schema}`：JSON数据的字段说明
* `{json_content}`：用户的投资计划数据（JSON格式）

### 实现文件
| 文件 | 说明 |
|------|------|
| `services/aiInvestmentDraftService.ts` | 投资计划AI分析服务 |
| `components/AIInvestmentDraftModal.tsx` | AI分析浮窗组件 |
| `components/InvestmentDraftModal.tsx` | 添加AI分析按钮入口 |
| `public/assets/config/ai-investment-draft-templates.json` | 提示词模板配置 |
...
## 组合交易组管理功能 (Combo Trade Group Management) — 新增功能说明 (v1.35)

提供一个组合交易管理界面，用户可以在该界面中对交易组合进行创建、编辑和删除操作。

### 界面入口

在基金交易明细窗口（TransactionsModal）内，表格的右上方，"批量输入"按钮的右边，增加一个"组合交易"按钮（图标使用 `fa-layer-group` 或类似）。用户点击后会打开一个新的窗口，显示组合交易的管理界面。

### 组合交易管理窗口

#### 已有组合列表
* 窗口首先显示已有的组合交易列表（panel），每个组合交易显示其名称，以链接的方式呈现，用户点击后会进入该组合交易的编辑界面。
* 每个组合交易旁边有一个删除按钮，用户点击后会弹出确认提示，确认后删除该组合交易。

#### 添加新组合
* 在组合交易列表的下方，提供一个输入框和一个"添加组合交易"按钮，用户输入新的组合交易名称后点击按钮，会创建一个新的组合交易，并刷新列表显示。

#### 编辑区域
* 再下方是组合交易编辑界面，用户选择一个组合交易后会进入该编辑界面（增加新的组合交易后会自动进入该编辑界面）。
* 编辑界面包含以下内容：
  * 组合交易名称输入框，用户可以修改组合交易的名称。
  * 一个输入表格，每一行代表一个在系统里有满仓份额的基金（`fullCapacity > 0`）：
    * 第一列是基金名称（只读）
    * 第二列是买入金额（数字输入框，默认为0，精确到小数点后2位）
    * 第三列是手续费（数字输入框，默认为0，精确到小数点后2位）
    * 第四列是操作（重置按钮）
  * 表格下方显示一个总计行，显示当前输入的组合交易的总条数、总买入金额和总手续费。
  * 编辑界面下方提供一个"保存"按钮。

#### 数据校验与保存
* 用户点击保存按钮后，系统会对输入的数据进行校验（组合名称不能为空）。
* 校验通过后，将交易记录保存到 localStorage 中对应的组合交易里。
* 保存时，只保存买入金额大于0的记录，买入金额为0的记录不保存。

#### 未保存数据提示
* 如果用户在没有点击保存按钮的情况下，直接选择了另一个组合交易或者点击了关闭窗口，则会弹出确认提示："当前输入的数据尚未保存，确定要放弃吗？"
  * 用户选择"确认放弃"：进入新的组合交易编辑界面或者关闭窗口，数据丢弃
  * 用户选择"取消"：保持当前编辑界面，数据保留

### 实现细节

* 窗口大小固定（约 `max-w-2xl`，`maxHeight: 90vh`），不会因为输入内容的多少而改变。
* 编辑区域的输入表格支持滚动，当输入的交易记录超过窗口可显示的范围时，用户可以通过滚动条查看和输入更多的条目。
* 用户在输入过程中，系统会实时计算并显示总计行的信息。
* 组合交易需要保存到 localStorage 中：
  * 存储键：`fund_combo_trades`
  * 每个组合交易都有自己唯一的 id（使用时间戳+随机数生成）
  * 组合交易的内容支持现有的备份导入导出功能
* 组合交易内部每条记录需要保存基金 id（而不是基金名称）、买入金额、手续费等信息。
  * 如果基金 id 在系统里找不到对应的基金，则在编辑界面显示"未知基金（基金id）"，但仍然显示该记录的买入金额和手续费。
* 编辑界面显示时，自动补齐买入金额为0的记录，使得用户可以看到所有具备满仓金额的基金。
* 记录排序：买入金额大于0的记录显示在输入表格的上方，买入金额为0的记录显示在下方。

### 类型定义

```typescript
interface ComboTradeRecord {
  fundId: string;      // 基金代码
  amount: number;      // 买入金额
  fee: number;         // 手续费
}

interface ComboTrade {
  id: string;          // 唯一标识
  name: string;         // 组合名称
  records: ComboTradeRecord[];  // 记录列表
}
```

### 备份集成

* 导出备份时，组合交易数据包含在 `BackupData.comboTrades` 字段中。
* 导入备份时，如果导入数据中包含组合交易数据，则完全覆盖现有的组合交易数据（导入时会丢弃 amount <= 0 的记录）。

### 实现文件

| 文件 | 改动 |
|-----|------|
| `types.ts` | 添加 `ComboTradeRecord` 和 `ComboTrade` 类型定义；`BackupData` 添加 `comboTrades` 字段 |
| `components/ComboTradeModal.tsx` | 新建：组合交易管理弹窗组件 |
| `components/TransactionsModal.tsx` | 添加"组合交易"按钮，渲染 ComboTradeModal |
| `utils/backupService.ts` | 添加 comboTrades 的导出和导入逻辑 |
| `components/config/BackupPanel.tsx` | 导入时正确处理 `comboTrades` 字段 |
...
## 批量交易录入 (Batch Trade Input) — 新增功能说明 (v1.36)

提供一个批量录入界面，用户可以在该界面中一次性输入某一天多个基金的多条交易记录。

### 界面入口
* 在基金交易明细窗口（TransactionsModal）内，表格的右上方，与日期选择平齐处，增加"批量输入"按钮，点击后打开交易批量输入窗口。

### 批量交易录入窗口
* 窗口中提供日期选择组件，默认为前一个交易日（从今天往前推，第一个非周末日期）。选择不同的日期后，窗口下方的输入表格会自动刷新。用户不能选择未来的日期。
* 日期选择下方提供一个可以收缩的组合交易面板：
  * 默认是收缩的，用户点击后展开。
  * 里面水平平铺所有的组合交易的名称，允许多行显示。
  * 当用户点击某个组合交易名称时，自动根据组合交易里面的每一条记录的基金id，在相应的基金分组里增加一条交易记录输入行，交易类型默认为"买入"，基金份额、手续费默认为组合交易里面该记录的金额和手续费。
* 提供一个输入表格，表格里面提供分组，每个分组里可以增加记录。
* 表格的每一个分组代表一个在系统里有满仓份额的基金。分组里显示基金的名称。最后面增加一个"+"按钮，用户点击后可以在该分组里增加一条交易记录输入行。
* 表格里面有下列列：交易类型（买入/卖出），交易价格，基金份额，手续费，总额，操作（此列不显示表头，里面显示删除按钮）。
  ...
  * 删除按钮：用户点击后删除该行输入。
* 表格下方提供一个总计行，包含总计买入交易条数、总计卖出交易条数、总计买入金额、总计卖出金额、总计手续费等信息。
...
* 输入表格支持滚动，当输入的交易记录超过窗口可显示的范围时，用户可以通过滚动条查看和输入更多的交易记录
* 用户在输入过程中，系统会实时计算并显示总计行的信息
...
- TransactionsModal（基金交易明细）
  ...
  - 表格（六列）：
    ...
    - 第六列"多选"：表头为全选/反选 checkbox，每行为单行选择 checkbox
    - 0 值全部显示黑色 `"-"`
    - 结构：外层固定 thead + `max-height: 400px` 滚动 tbody + 外层固定 tfoot 统计行
    - 选定日期无交易时 tbody 显示"该日期无任何交易"
  - 统计行（固定在底部，与批量交易录入窗口样式一致）：
    - 左侧：`总计：买入 N 条，卖出 M 条`
    - 右侧：买入总额、卖出总额、手续费（均显示千分位两位小数）
  - 列宽（table-fixed）：基金名称 25%、类型 10%、份额 13%、手续费 12%、交易总额 18%、多选 6%
