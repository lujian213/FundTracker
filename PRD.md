...
* 在主界面的右上角的下拉菜单中，在"AI配置"下面加入一个"系统开关"菜单项，点击后弹出系统开关窗口。
* 扳手图标按钮仅在以下条件同时满足时显示：初始份额 > 0，且系统开关中的功能已启用。

### 初始价格调整窗口
* 窗口标题为"调整初始价格"。
* 第一排显示：目前盈利（只读输入框，正数红色、负数绿色）、目前初始价格。
* 第二排显示：参考盈利（输入框，默认值为目前盈利，正数红色、负数绿色）、建议初始价格（只读）。
* 第三排居中显示"保存"按钮。
* 所有数字保留两位小数。
...
### 实现细节
* 计算公式：初始价格 = ((当前份额 × 当前价格) + 卖出总额 - 买入总额 - 参考盈利) / 初始份额
