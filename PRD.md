...
```
* 复制图标左侧有一个刷新图标，点击后可以刷新表格中的估值、前值和盈亏百分比，以及换算出来的份额。刷新时会保留用户在操作列和金额列的输入。
...
#### 功能特性
1. **AI助手按钮**：在基金详情页面的工具栏中添加一个AI机器人图标按钮
2. **侧边面板**：点击按钮后，一个滑入的侧边面板显示AI聊天界面
3. **上下文感知**：AI助手自动获取当前基金的实时数据（价格、涨跌幅、净值等）并将其包含在对话上下文中
4. **可配置API**：用户可以配置自己的AI服务API端点和密钥
5. **会话保持**：用户可以在同一会话中进行连续对话，AI记住上下文

#### 界面交互
- **AI按钮位置**：位于基金详情页面的工具栏中，计算器按钮和虚拟交易按钮之间
- **侧边面板**：从右侧滑入，占据屏幕高度，与基金详情页面平行显示
- **配置界面**：通过侧边面板顶部的齿轮图标访问，允许用户设置API端点、密钥和模型类型
- **聊天界面**：显示消息历史，支持文本输入和发送，包含加载指示器

#### 配置要求
- **API端点**：用户需要提供兼容OpenAI格式的API端点URL
- **API密钥**：安全存储在localStorage中，不包含在备份中
- **模型选择**：可选的模型名称（默认：gpt-4）

#### 数据流
1. 用户点击基金详情页面的AI助手按钮
2. 应用收集当前基金的相关数据（名称、代码、估值、净值等）
3. 侧边面板打开并初始化聊天界面
4. 初始基金信息自动发送给AI以获取基金概览
5. 用户可以提出后续问题，系统将基金上下文包含在每个请求中
6. AI响应显示在聊天界面中

#### 安全考虑
- API密钥仅存储在localStorage中，与应用的其他敏感数据使用相同安全级别
- 除了配置的AI端点外，不传输任何数据到外部服务器
- 配置验证以防止注入攻击
- 密钥以密码字段形式隐藏输入
