...
* 表格下方对数量、交易额和盈利额进行合计展示，合计行的背景色为灰色，合计行的日期列显示”合计”，其他列显示”-”。
  ...
  * 操作列的合计值仅在普通视图中展示，显示所有交易的总次数（买入+卖出+建仓）。鼠标悬停时显示”买入/建仓 x 次，卖出 y 次”，其中x为所有买入或建仓的交易总次数，y为所有卖出交易总次数。先进先出和后进先出视图中显示”-”。
  * 交易额列的合计值仅在普通视图中展示，先进先出和后进先出视图中不展示（因为这两种视图以份额对冲，交易额合计无意义）。合计值为所有买入和建仓记录的交易额总和减去所有卖出记录的交易额总和。颜色为黑色，显示绝对值。正数时在数值后加向上箭头图标（红色），鼠标悬停时显示”总买入 xxx, 总卖出 yyy, 净投入 zzz”；负数时加向下箭头图标（绿色），鼠标悬停时显示”总买入 xxx, 总卖出 yyy, 净回收 zzz”；0时显示”0.00”不带图标。所有金额保留两位小数，以千分位分隔符展示。其中，总买入为所有买入和建仓记录的交易额总和，总卖出为所有卖出记录的交易额总和。
