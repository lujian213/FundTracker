...
* 在手续费输入框内（靠右侧边缘）增加一个辅助输入按钮，按钮图案为一个计算器，代表“自动计算”。点击后，根据历史最近一条相同类型的交易，计算相应的手续费的值并填充。用户可以根据实际情况调整手续费值。
  * 如果没有历史相同类型的交易记录，则输入0.
  * 如果为买入交易，则根据最近一条买入记录的手续费率（手续费/交易额）进行计算。手续费 = 当前总额 * 最近一条买入记录的手续费率。
  * 如果为卖出交易，则根据最近一条卖出记录的手续费率（手续费/交易额）进行计算。手续费 = 当前交易价格 * 卖出数量 * 最近一条卖出记录的手续费率。
