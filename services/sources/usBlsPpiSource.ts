// services/sources/usBlsPpiSource.ts
...
/**
 * BLS PPI 数据源
 * 从美国劳工统计局（BLS）官网获取 PPI 发布日程
 *
 * 数据源页面：https://www.bls.gov/schedule/news_release/ppi.htm
 */
export class UsBlsPpiSource extends ImportantDataSourceBase {
  constructor() {
    const config: ImportantDataSourceConfig = {
      eventType: 'important_data_us_ppi',
      eventName: 'PPI数据公布',
      market: '美股',
  ...
  /**
   * 获取 BLS PPI 发布日程页面 URL
   */
  getSourceUrl(): string {
    return 'https://www.bls.gov/schedule/news_release/ppi.htm';
  }
