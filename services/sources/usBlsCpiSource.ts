// services/sources/usBlsCpiSource.ts
...
/**
 * BLS CPI 数据源
 * 从美国劳工统计局（BLS）官网获取 CPI 发布日程
 *
 * 数据源页面：https://www.bls.gov/schedule/news_release/cpi.htm
 */
export class UsBlsCpiSource extends ImportantDataSourceBase {
  ...
  parseHtml(html: string): ImportantDataEventInfo[] {
    ...
    // 尝试匹配表格格式
    // <tr><td>June 2026</td><td>July 14, 2026</td><td>8:30 A.M.</td></tr>
    ...
    if (events.length === 0) {
      ...
      // 匹配列表格式：
      // "June 2026 - July 14, 2026 at 8:30 A.M."
  ...
  parseMarkdown(markdown: string): ImportantDataEventInfo[] {
    ...
    while ((match = rowPattern.exec(tableContent)) !== null) {
      ...
      // 注意：BLS 使用缩写月份，如 "Dec."、"Jan."
      const releaseMatch = releaseDateStr.match(/^(\w+)\.?\s+(\d{1,2}),?\s+(\d{4})$/i);
      if (!releaseMatch) {
        continue;
      }

      const releaseMonthStr = releaseMatch[1].replace(/\.$/, ''); // 去掉可能的点号后缀（如 "Dec." -> "Dec")
