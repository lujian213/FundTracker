...
test.describe('Testbed with loaded data', () => {
  ...
  test('基金持仓窗口：饼图和表格显示21只基金，趋势图基本验证', async () => {
    const page = sharedPage!;
    ...
    if (chartInfo.dataPointCount > 0) {
      ...
      if (chartBounds) {
        ...
        // 验证虚线出现（hover时的垂直辅助线）
        const dashedLine = page.locator('svg line[stroke-dasharray]');
        expect(await dashedLine.count()).toBeGreaterThan(0);
      }
    }
