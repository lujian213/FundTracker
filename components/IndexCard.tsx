...
const IndexCard: React.FC<IndexCardProps> = ({
  ...
  const shouldShowHistoryLabel = (() => {
    if (!idx.tradeDate) return false;
    const now = new Date();
    const todayStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
    return idx.tradeDate < todayStr;
  })();

  // 格式化交易日期显示 (MM/DD)
  const formattedTradeDate = idx.tradeDate
    ? `${idx.tradeDate.substring(5, 7)}/${idx.tradeDate.substring(8, 10)}`
    : '';

  // 格式化完整日期时间显示 (MM-dd HH:mm)
  const formattedDateTime = idx.tradeDate
    ? `${idx.tradeDate.substring(5, 7)}-${idx.tradeDate.substring(8, 10)} ${idx.lastUpdated.substring(0, 5)}`
    : idx.lastUpdated.substring(0, 5);

  // 格式化前值（千分位，2位小数）
  const formattedPreviousClose = idx.previousClose
    ? idx.previousClose.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    : '-';

  const sparkline = useMemo(() => buildSparklinePath(getIntradayPoints(idx.symbol)), [idx.symbol, idx.lastUpdated]);
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1 min-w-0 pr-2">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <p className="text-[9px] text-gray-400 font-mono">{idx.symbol}</p>
              {sparkline && (
