// components/NewsAIAnalysisModal.tsx
...
const NewsAIAnalysisModal: React.FC<NewsAIAnalysisModalProps> = ({
  isVisible,
  onClose,
  news
}) => {
  ...
  const performAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const result = await analyzeNewsImpact(config, news, funds, handleChunk);
  ...
  const renderNotConfigured = () => (
    <div className="flex flex-col items-center justify-center py-12 text-gray-500">
      ...
      <p className="text-sm text-gray-400 mb-4">请先完成AI配置以使用快讯分析功能</p>
      <button
        onClick={() => {
          // 触发打开AI配置的事件
  ...
  const renderLoading = () => (
    <div className="flex flex-col items-center justify-center py-12">
      ...
      <p className="text-gray-600">AI正在分析快讯影响...</p>
    </div>
  );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      >
        ...
        >
          <h3 id="ai-news-title" className="text-lg font-bold pointer-events-none">AI 快讯影响分析</h3>
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto p-6 min-h-0">
          {/* 快讯预览 */}
          {hasUsableAIConfig() && renderNewsPreview()}
          {/* 分析内容 */}
          {renderMainContent()}
...
export default NewsAIAnalysisModal;