...
export const FundDetailsModal: React.FC<FundDetailsModalProps> = ({ data, onClose }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setHistory(points.slice(-90)); // 最近90天
      setLoading(false);
    };
    load();
  }, [data.symbol]);
  ...
  const chartData = useMemo(() => {
    ...
    const valuationTs = new Date(data.valuationDate).getTime();
  ...
  const { path, area, points, viewBox, yLabels, xLabels } = useMemo(() => {
    ...
    const width = 1000;
    const height = 450; // 增加高度以留出 X 轴标签空间
    const paddingLeft = 60; // 留出 Y 轴空间
    const paddingRight = 30;
    const paddingTop = 40;
    const paddingBottom = 60; // 留出 X 轴空间

    const values = chartData.map(p => p.value);
    const rawMin = Math.min(...values);
    const rawMax = Math.max(...values);
    const margin = (rawMax - rawMin) * 0.1 || 0.01; // 增加 10% 边距
        ...
        <div className="px-6 py-6 border-b border-gray-50 flex justify-between items-start">
          <div>
            ...
            <div className="flex items-baseline space-x-3">
              <span className={`text-2xl font-black ${data.changePercentage >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                {data.currentPrice.toFixed(4)}
              </span>
              <span className={`text-sm font-bold ${data.changePercentage >= 0 ? 'text-red-500' : 'text-green-500'}`}>
                {data.changePercentage >= 0 ? '+' : ''}{data.changePercentage.toFixed(2)}%
              </span>
              <span className="text-[10px] text-gray-400 font-medium">昨日净值: {data.previousPrice.toFixed(4)}</span>
            </div>
          </div>
            ...
            <div className="space-y-6">
              <div className="relative bg-gray-50 rounded-2xl p-4">
                <div className="absolute top-4 left-4 z-10">
                   ...
                   {hoveredPoint ? (
                     <div className="animate-in fade-in slide-in-from-left-2 duration-150">
                        <p className="text-lg font-black text-gray-800">{hoveredPoint.value.toFixed(4)}</p>
                        <p className="text-[10px] text-gray-500 font-bold">
                           {new Date(hoveredPoint.date).toLocaleDateString()}
                           <span className={`ml-2 ${hoveredPoint.equityReturn >= 0 ? 'text-red-500' : 'text-green-500'}`}>
                ...
                >
                  ...
                  {yLabels.map((label, i) => (
                    <g key={i}>
                      <line
                        x1="60" y1={label.y} x2="970" y2={label.y}
                      ...
                      <text
                        ...
                        className="text-[22px] fill-gray-400 font-mono"
                      >
                        {label.text}
                      </text>
                    </g>
                  ...
                  {xLabels.map((label, i) => (
                    <text
                      ...
                      y="420"
                      textAnchor="middle"
                      className="text-[22px] fill-gray-400 font-medium"
                    >
                      {label.text}
                    </text>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="p-4 bg-gray-50 rounded-2xl">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">更新时间</p>
                 ...
                 <div className="p-4 bg-gray-50 rounded-2xl">
                    ...
                    <p className="text-sm font-bold text-gray-700">{data.valuationDate}</p>
                 </div>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block w-full py-4 text-center text-xs font-bold text-gray-400 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-colors"
              >
